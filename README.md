...
[![DevSecOps](https://img.shields.io/badge/Trivy-Vulnerability_Scanner-red.svg)](https://aquasecurity.github.io/trivy/)
...
This repository houses the source code and fully automated continuous delivery pipeline for **[The Libre Life](https://thelibrelife.org)**. It serves as a proof-of-concept for enterprise-grade GitOps and DevSecOps, demonstrating a single GitHub Actions workflow that simultaneously deploys a highly-optimized static application to a global CDN, while securely packaging, scanning, and pushing a production-ready Docker container to a private AWS cloud registry.
...
flowchart TD
    ...
    subgraph CI_CD ["CI/CD Pipeline (GitHub Actions)"]
        ...
        Trigger --> Job2[Job 2: Build, Scan & Push Docker Image]
        ...
        subgraph DockerPipeline ["Target 2: Enterprise Artifact (DevSecOps)"]
            Job2 --> Auth[Authenticate AWS IAM]
            Auth --> BuildImg[Build Multi-stage Nginx Container]
            BuildImg --> Trivy{Trivy Vulnerability Scan}
            Trivy -->|Fail| Halt[Block Push: CRITICAL/HIGH CVEs found]
            Trivy -->|Pass| TagImg[Tag Image with Git SHA]
        end
    end
    ...
    style Trivy fill:#ffe6e6,stroke:#cc0000,stroke-width:2px,color:#000
    style Halt fill:#ffcccc,stroke:#cc0000,stroke-width:2px,color:#000
...
* **Target 2 (Enterprise Artifact & SecOps):** Authenticates with AWS IAM via secure secrets, executes a multi-stage Docker build, and runs an automated **Trivy vulnerability scan**. If the image passes the security gate (no HIGH/CRITICAL CVEs), the immutable, Nginx-backed container is pushed to a private Amazon Elastic Container Registry (ECR) in the `eu-north-1` (Stockholm) region.
...
* **CI/CD & GitOps:** GitHub Actions (Dual-Target Matrix)
* **DevSecOps:** Aqua Security Trivy (Automated container vulnerability scanning)
...
* **Shifting Security Left (DevSecOps):** Injecting security at the end of a deployment cycle is an anti-pattern. **Solution:** Integrated Aqua Security's Trivy scanner directly into the GitHub Actions runner. The pipeline is explicitly configured to fail and block pushes to AWS ECR if any `CRITICAL` or `HIGH` vulnerabilities are detected in the OS or library layers.
